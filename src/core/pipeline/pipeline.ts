...
/**
 * Throws if `result` contains a Map as a direct property value.
 * Maps are silently dropped by JSON.stringify — use plain objects (Record) instead.
 */
function assertNoMaps(result: unknown): void {
  ...
  for (const value of Object.values(result)) {
    if (value instanceof Map) {
      throw new Error(
        "State contains a Map, which cannot be serialized to JSON. Use a plain object (Record) instead.",
      );
    }
  }
}
